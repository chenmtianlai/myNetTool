...
func CCC_postCrypto_DDD(_ dic : NSDictionary, _ path : String) -> String {
    ...
    let kkk_value_lll = CCC_hmacBase64_DDD(key: kkk_signKey_lll, message: kkk_signContent_lll)

    let kkk_key_lll = String(format: "type=0;key=%@;time=%d", CCC_base64_DDD(kkk_signKey_lll), kkk_time_lll)
    let kkk_data_lll = CCC_rsaEncrypt_DDD(kkk_key_lll)

    let kkk_latestValue_lll = "key=" + kkk_fingerprint_lll + ";secret=" + kkk_data_lll + ";signature=" + kkk_value_lll
    return kkk_latestValue_lll
}



func CCC_getCrypto_DDD(_ dic : NSDictionary, _ path : String) -> String {
...
//排序，拼接，
func CCC_getBodySign_DDD(_ dic : NSDictionary) -> String {
    ...
    let kkk_request_lll = AF.request(kkk_baseUrl_lll,method: .get,parameters: (dic as! Parameters),headers: kkk_headerDic_lll) { requests in}
    
    let kkk_range_lll = NSRange(location: 0, length: 0)
    var kkk_urls1_lll = String(format: "%@", (kkk_request_lll.convertible.urlRequest?.url?.absoluteString as? CVarArg) ?? "")
    kkk_urls1_lll = kkk_urls1_lll.replacingOccurrences(of: kkk_baseUrl_lll + "?", with: "")
    return kkk_urls1_lll
}

func CCC_getBodySign1_DDD(_ dic : NSDictionary) -> String {
...
func CCC_base64_DDD(_ string: String) -> String {
...
//rsa加密
func CCC_rsaEncrypt_DDD(_ text: String) -> String {
    do {
        let encrypted = try AAA_RSAEncryptor_BBB.CCC_encrypt_DDD(text, kkk_publicKey_lll)
        return encrypted
    } catch {
        return ""
    }
...
func CCC_hmacBase64_DDD(key: String, message: String) -> String {
...
func CCC_sha256Encrypt_DDD(_ input: String) -> String {
...
class AAA_RSAEncryptor_BBB {
    ...
    private static func CCC_preparePublicKey_DDD(_ key: String) throws -> SecKey {
        ...
        var kkk_keyString_lll = key.replacingOccurrences(of: kkk_header_lll, with: "")
